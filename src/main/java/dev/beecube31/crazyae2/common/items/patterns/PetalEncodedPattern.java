...
import dev.beecube31.crazyae2.common.util.patterns.crafting.PetalCraftingPatternDetails;
...
public class PetalEncodedPattern extends ItemCustomEncodedPatternBase implements ICraftingPatternItem {
    ...
    public ICraftingPatternDetails getPatternForItem(final ItemStack is, final World w) {
        try {
            return new PetalCraftingPatternDetails(is);
    ...
    protected boolean clearPattern(final ItemStack stack, final EntityPlayer player) {
        if (player.isSneaking()) {
            ...
            ItemStack is = CrazyAE.definitions().materials().petalBlankPattern().maybeStack(stack.getCount()).orElse(ItemStack.EMPTY);
            if (!is.isEmpty()) {
