...
import dev.beecube31.crazyae2.common.util.patterns.crafting.ElventradeCraftingPatternDetails;
...
public class ElventradeEncodedPattern extends ItemCustomEncodedPatternBase implements ICraftingPatternItem {
    ...
    public ICraftingPatternDetails getPatternForItem(final ItemStack is, final World w) {
        try {
            return new ElventradeCraftingPatternDetails(is);
    ...
    protected boolean clearPattern(final ItemStack stack, final EntityPlayer player) {
        if (player.isSneaking()) {
            ...
            ItemStack is = CrazyAE.definitions().materials().elventradeBlankPattern().maybeStack(stack.getCount()).orElse(ItemStack.EMPTY);
            if (!is.isEmpty()) {
