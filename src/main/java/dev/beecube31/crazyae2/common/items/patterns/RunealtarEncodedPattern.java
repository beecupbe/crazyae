...
public class RunealtarEncodedPattern extends ItemCustomEncodedPatternBase implements ICraftingPatternItem {
    ...
    public ICraftingPatternDetails getPatternForItem(final ItemStack is, final World w) {
        try {
            return new RunealtarCraftingPatternDetails(is);
    ...
    protected boolean clearPattern(final ItemStack stack, final EntityPlayer player) {
        if (player.isSneaking()) {
            ...
            ItemStack is = CrazyAE.definitions().materials().runealtarBlankPattern().maybeStack(stack.getCount()).orElse(ItemStack.EMPTY);
            if (!is.isEmpty()) {
