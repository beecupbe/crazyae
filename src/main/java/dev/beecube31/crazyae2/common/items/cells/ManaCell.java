...
public abstract class ManaCell<T extends IAEStack<T>> extends AEBaseItem implements IStorageCell<T>, IItemGroup {
	protected final Materials.MaterialType component;
	protected final int capacity;
	protected final double idleDrain;
	protected final int bytesPerType;

	public ManaCell(Materials.MaterialType whichCell, int bytes) {
		this(whichCell, bytes, 1, 16);
	}

	public ManaCell(Materials.MaterialType whichCell, int bytes, int bytesPerType) {
		this(whichCell, bytes, bytesPerType, 16);
	}

	public ManaCell(Materials.MaterialType whichCell, int bytes, double idleDrain) {
		this(whichCell, bytes, 1, idleDrain);
	}

	public ManaCell(Materials.MaterialType whichCell, int bytes, int bytesPerType, double idleDrain) {
	...
	public String getUnlocalizedGroupName(Set<ItemStack> others, ItemStack is) {
		return CrazyAEGuiText.MANA_CELLS.getUnlocalized();
	}
