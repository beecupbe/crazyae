...
public class ManapoolEncodedPattern extends ItemCustomEncodedPatternBase implements ICraftingPatternItem {
    ...
    public ICraftingPatternDetails getPatternForItem(final ItemStack is, final World w) {
        try {
            return new ManapoolCraftingPatternDetails(is);
    ...
    protected boolean clearPattern(final ItemStack stack, final EntityPlayer player) {
        if (player.isSneaking()) {
            ...
            ItemStack is = CrazyAE.definitions().materials().manapoolBlankPattern().maybeStack(stack.getCount()).orElse(ItemStack.EMPTY);
            if (!is.isEmpty()) {
