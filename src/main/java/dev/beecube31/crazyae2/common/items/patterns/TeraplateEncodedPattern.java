...
public class TeraplateEncodedPattern extends ItemCustomEncodedPatternBase implements ICraftingPatternItem {
    ...
    public ICraftingPatternDetails getPatternForItem(final ItemStack is, final World w) {
        try {
            return new TeraplateCraftingPatternDetails(is);
    ...
    protected boolean clearPattern(final ItemStack stack, final EntityPlayer player) {
        if (player.isSneaking()) {
            ...
            ItemStack is = CrazyAE.definitions().materials().teraplateBlankPattern().maybeStack(stack.getCount()).orElse(ItemStack.EMPTY);
            if (!is.isEmpty()) {
