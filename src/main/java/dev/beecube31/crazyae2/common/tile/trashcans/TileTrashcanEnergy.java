...
public class TileTrashcanEnergy extends TileTrashcanBase {
    ...
    public void onChangeInventory(IItemHandler inv, int slot, InvOperation mc, ItemStack removed, ItemStack added) {
        ...
        if (inv != this.inv) {
            if (Utils.getItemEnergyType(this.inv.getStackInSlot(slot)) == null) {
        ...
        try {
            ...
            for (IAEItemStack candidate : this.inv) {
                ...
                if (o != null && o.getStackSize() > 0) {
                    IAEItemStack ais = AEUtils.extractFromME(inv, o.setStackSize(this.itemsToSendPerTick), this.src, Actionable.MODULATE);
                    if (ais.getStackSize() < Long.MAX_VALUE) {
                        worked = true;
                    }
                }
            }
