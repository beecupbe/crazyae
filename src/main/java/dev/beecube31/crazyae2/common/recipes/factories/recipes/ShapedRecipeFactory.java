package dev.beecube31.crazyae2.common.recipes.factories.recipes;
...
public class ShapedRecipeFactory implements IRecipeFactory {
	...
	public ShapedOreRecipe parse(JsonContext context, JsonObject json) {
		...
		if (Objects.equals(type, "part")) {
			var partDef = CrazyAE.definitions().parts().getById(name).orElse(null);
			...
			var materialDef = CrazyAE.definitions().materials().getById(name).orElse(null);
