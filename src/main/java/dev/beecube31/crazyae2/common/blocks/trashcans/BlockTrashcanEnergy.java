...
public class BlockTrashcanEnergy extends CrazyAEBlockAttribute implements ICustomCollision {
    ...
    // Пример реализации функции для объединения всех AABB в один (для getBoundingBox)
    // public static AxisAlignedBB calculateUnionOfBoxes(List<AxisAlignedBB> boxes) {
    //     if (boxes == null || boxes.isEmpty()) {
    //         return Block.FULL_BLOCK_AABB; // Или null, или какой-то стандартный AABB
    //     }
    ...
    public boolean onActivated(final World w, final BlockPos pos, final EntityPlayer p, final EnumHand hand, final @Nullable ItemStack heldItem, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
        ...
        if (tg != null) {
            if (Platform.isServer()) {
                CrazyAEGuiHandler.openGUI(p, tg, AEPartLocation.fromFacing(side), CrazyAEGuiBridge.GUI_TRASHCAN_ENERGY);
            }
            return true;
        }
        return false;
