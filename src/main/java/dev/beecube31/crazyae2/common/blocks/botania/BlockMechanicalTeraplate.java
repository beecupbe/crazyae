...
public class BlockMechanicalTeraplate extends CrazyAEModelBlock {

    public BlockMechanicalTeraplate() {
        super(Material.IRON);
    }
    ...
    public boolean onActivated(final World w, final BlockPos pos, final EntityPlayer player, final EnumHand hand, final @Nullable ItemStack heldItem, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
        ...
        if (Platform.isServer()) {
            final TileMechanicalTerraplate tc = this.getTileEntity(w, pos);
            if (tc != null && !player.isSneaking()) {
                CrazyAEGuiHandler.openGUI(player, tc, AEPartLocation.fromFacing(side), CrazyAEGuiBridge.GUI_TERAPLATE_MECHANICAL);
                return true;
            }
        }
