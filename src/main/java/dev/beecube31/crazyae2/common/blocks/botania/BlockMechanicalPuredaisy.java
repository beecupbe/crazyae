...
public class BlockMechanicalPuredaisy extends CrazyAEModelBlock {

    public BlockMechanicalPuredaisy() {
        super(Material.IRON);
    }
    ...
    public boolean onActivated(final World w, final BlockPos pos, final EntityPlayer player, final EnumHand hand, final @Nullable ItemStack heldItem, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
        ...
        if (Platform.isServer()) {
            final TileMechanicalPuredaisy tc = this.getTileEntity(w, pos);
            if (tc != null && !player.isSneaking()) {
                CrazyAEGuiHandler.openGUI(player, tc, AEPartLocation.fromFacing(side), CrazyAEGuiBridge.GUI_PUREDAISY_MECHANICAL);
                return true;
            }
        }
