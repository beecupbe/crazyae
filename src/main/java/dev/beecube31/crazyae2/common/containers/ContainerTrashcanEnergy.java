...
public class ContainerTrashcanEnergy extends ContainerCrazyAEUpgradeable {
    ...
    public ContainerTrashcanEnergy(final InventoryPlayer ip, final TileTrashcanBase te) {
        super(ip, te);
    }
    ...
    protected void setupConfig() {
        ...
        for (int y = 0; y < 8; y++) {
            for (int x = 0; x < 9; x++) {
                this.addSlotToContainer((new RestrictedSlot(RestrictedSlot.PlaceableItemType.ENERGY_STACKS, config, x + y * 9, 8 + x * 18, 23 + y * 18, this.getInventoryPlayer())).setStackLimit(1).setNotDraggable());
            }
        }
