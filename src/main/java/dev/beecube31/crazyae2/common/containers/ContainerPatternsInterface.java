...
public class ContainerPatternsInterface extends ContainerCrazyAEUpgradeable implements IOptionalSlotHost {
    ...
    public ContainerPatternsInterface(final InventoryPlayer ip, final IInterfaceHost te) {
        ...
        for (int row = 0; row < 8; ++row) {
            for (int x = 0; x < 9; x++) {
                this.addSlotToContainer(new RestrictedSlot(RestrictedSlot.PlaceableItemType.ENCODED_PATTERN, this.myDuality
                        .getPatterns(), x + row * 9, 8 + 18 * x, 25 + (18 * row), this.getInventoryPlayer()).setStackLimit(1));
            }
        }
        for (int i = 0; i < 3; i++) {
            for (int x = 0; x < 3; x++) {
                this.addSlotToContainer(new CrazyAESlot(this.myDuality.getStorage(), x + i * 3, 187 + (18 * x), 99 + (18 * i)));
            }
        }
    }
