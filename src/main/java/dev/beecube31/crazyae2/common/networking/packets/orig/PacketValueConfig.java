...
public class PacketValueConfig extends CrazyAEPacket {
    ...
    public void clientPacketData(final INetworkInfo network, final CrazyAEPacket packet, final EntityPlayer player) {
        ...
        if (this.Name.equals("CustomName") && c instanceof CrazyAEBaseContainer) {
            ((CrazyAEBaseContainer) c).setCustomName(this.Value);
        } else if (this.Name.startsWith("SyncDat.")) {
            ((CrazyAEBaseContainer) c).stringSync(Integer.parseInt(this.Name.substring(8)), this.Value);
